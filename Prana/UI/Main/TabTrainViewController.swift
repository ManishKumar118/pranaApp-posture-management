    ...
    func calculateSummary() {
        ...
        if let sessions = dataController?.fetchDailySessions(date: Date()).filter({ (object) -> Bool in
            ...
            if postureElapsed > 0 {
                ...
                UserDefaults.standard.set(breathingElapsedFloored, forKey: "breathingElapsedFloored")
               UserDefaults.standard.set(postureElapsedFloored, forKey: "postureElapsedFloored")
                let breathing = UserDefaults.standard.value(forKey: "breathingElapsedFloored")
                let posture = UserDefaults.standard.value(forKey: "postureElapsedFloored")
               lblMindfulBreathTime.text = "\(breathing ?? 0)"
               lblUprightPostureTime.text = "\(posture ?? 0)"
